...
- Your MCP endpoint URL (e.g. `https://abc123.droplet-mcp.nz/mcp`)
...
To reinforce the most important arcs, pass a `train` array (2–3 passes) in a
follow-up `session_checkpoint` — there is no separate `memory_train` tool, and
training skips atoms that aren't persisted yet.
...
session_checkpoint({
  atoms: [
    { atom: "v1.fact.database_host", payload: "prod.example.com:5432 (db: myapp)" },
    { atom: "v1.state.current_task", payload: "Implementing payment webhook handler" }
  ],
  edges: [
    { source: "v1.state.current_task", target: "v1.fact.database_host", type: "depends_on" }
  ]
})
\`\`\`
```
...
- `v1.fact.stripe_webhook_secret_location` — In env as `STRIPE_WEBHOOK_SECRET` (the secret itself is never stored in memory — credential-like values are rejected on write)
- `v1.procedure.always_verify_stripe_sig` — Verify the `Stripe-Signature` header before processing any event
- `v1.event.webhook_deployed_2026_03_15` — Deployed webhook to production; all events firing correctly

**Claude:** "Welcome back. I can see we were working on the payment webhook. Last session we deployed it successfully, and the signature-verification procedure is stored. The webhook secret itself stays in your env — memory never keeps credentials. Where would you like to pick up?"
