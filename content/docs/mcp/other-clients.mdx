...
description: Connect Parametric Memory to Cursor, Cline, Windsurf, Continue, and any other MCP-compatible AI tool with a single config block.
...
|---|---|
| **MCP URL** | `https://abc123.droplet-mcp.nz/mcp` |
| **API Key** | Dashboard → Settings → API Key (starts with `mmpm_live_`) |
...
URL:        https://abc123.droplet-mcp.nz/mcp
Auth:       Authorization: Bearer mmpm_live_your_key_here
```
...
Body: { "apiKey": "mmpm_live_your_key_here" }
...
- `session_checkpoint` — save atoms and relationships during a session (reinforcement is its `train` array — there is no separate `memory_train` tool)
- `memory_search` — semantic search across stored atoms
- `memory_access` — recall a specific atom by key
