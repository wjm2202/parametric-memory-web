...
| `markovScale` | number | Markov spreading-activation multiplier. Default `0.25` (on by default; set `0` to disable); try `0.25`–`1.0` when density is high. |
...
Precondition diagnostic for `memory_session_bootstrap`'s `markovScale`. With `markovScale` at its default `0.25`, a density below ~10% leaves the spread little to work from and higher values add little — the Markov wire has too little to spread from.
