 ...
 * no jsdom — so the same code runs under the CLI runner (tsx/node) and the
 * standalone vitest config in this folder.
 *
 * Nothing here does IO. Fetching lives in aeo-audit.ts. That separation is
 * what makes the audit logic testable.
 *
 * NOTE: this whole folder is deliberately EXCLUDED from CI/CD — it is not part
 * of `npm test`, `guard:all`, or `preflight`. See scripts/aeo/README.md.
 */
...
/**
 * Extract page paths from a sitemap.xml body, relative to `base`.
 * Used by the runner's `--sitemap` mode so "audit every page" stays in sync
 * with the sitemap as pages are added. Off-host URLs are dropped; results are
 * de-duplicated and returned as paths (e.g. "/", "/faq").
 */
export function sitemapPaths(xml: string | null, base: string): string[] {
  if (!xml) return [];
  const host = (() => {
    try {
