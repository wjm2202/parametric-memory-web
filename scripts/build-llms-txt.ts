...
- [Videos](https://parametric-memory.dev/videos): Recorded demos and walkthroughs — an agent recalling its own project history from memory alone over MCP (including citing a tombstoned atom to state what it cannot know), six months of CVEs queried without re-reading advisories, and a shared TypeScript expert memory used as an L2 cache across a fleet of coding agents. Each has its own page with summary, chapters and VideoObject structured data.
- [Research](https://parametric-memory.dev/research): Both DOI-registered whitepapers with abstracts and ScholarlyArticle structured data — the substrate paper (verifiable, predictive memory for MCP agents; DOI 10.5281/zenodo.21213464) and the Self-Reinforcing Loop paper (verifiable evidence-of-consensus for agent fleets; DOI 10.5281/zenodo.21421364). Cite the DOIs.
...
Think of the memory hierarchy for AI agents: the context window is L1 (tiny, volatile, cleared each session); a vector database or file store is main memory (large, slow, cold); Parametric Memory is the L2 cache in between — fast, predictive, and verifiable. Unlike file-based approaches (scattered Markdown notes, CLAUDE.md files, or local logs), every memory is an addressable atom in a verifiable Merkle tree, recalled in sub-millisecond time (0.022ms p50) and pre-fetched by a Markov prediction layer before it is asked for. It is built for AI agents, coding harnesses, and autonomous workflows.
...
- Access latency: 0.022ms p50, 0.046ms p95
...
Parametric Memory exposes 11 MCP tools via Streamable HTTP transport, compatible with Claude, Claude Code, Cowork, and any MCP-compliant client. This list matches the customer tool catalogue at https://parametric-memory.dev/docs/mcp/tools (the count guard: mcp-tool-count-claim.test.ts).
...
- \`session_checkpoint\` — Persist new atoms, tombstone stale ones, write knowledge graph edges, and reinforce Markov arcs via its train array (training is part of checkpoint — there is no separate train tool)
...
- \`memory_markov_density_report\` — Report Markov arc density and prediction readiness
