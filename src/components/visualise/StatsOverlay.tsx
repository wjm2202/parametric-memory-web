...
export default function StatsOverlay() {
  ...
  return (
    <>
      ...
      {/* Top-left: tree stats — pushed below nav bar */}
      <div className="absolute top-14 left-6 space-y-3">
        <div className="flex items-center gap-2">
          ...
          {/* S16-3: SSE status */}
          {sseStatus === "connected" && (
            <span className="ml-2 rounded-full bg-violet-500/15 px-2 py-0.5 font-mono text-[10px] tracking-wider text-violet-400 ring-1 ring-violet-500/30">
              SSE LIVE
            </span>
          )}
          {sseStatus === "connecting" && (
            <span className="ml-2 font-mono text-[10px] text-amber-400/60">SSE…</span>
          )}
          {sseStatus === "fallback" && (
            <span className="ml-2 font-mono text-[10px] text-slate-500">POLL</span>
          )}
        </div>
        {/* S16-5: Connected clients count */}
        ...
        {/* Position resolution progress is now shown as a top-center banner */}
      ...
      {/* Top-right: legend — pushed below nav bar */}
      <div className="absolute top-14 right-6 space-y-1.5">
      ...
      <div className="absolute bottom-6 left-6 flex items-center gap-3">
        ...
        >
          {sceneErrors.length > 0
            ? `⚠ ${sceneErrors.length} ERROR${sceneErrors.length > 1 ? "S" : ""}`
            : "LOG"}
        </button>
      </div>
      ...
      {/* Error log panel */}
      {showErrorLog && (
        <div className="absolute bottom-16 left-6 max-h-64 w-[480px] overflow-y-auto rounded-lg border border-slate-700/50 bg-slate-900/95 p-3 shadow-2xl backdrop-blur-md">
          ...
          {sceneErrors.length === 0 ? (
            ...
            <div className="space-y-1">
              {[...sceneErrors].reverse().map((e, i) => (
                <div key={`${e.timestamp}-${i}`} className="font-mono text-[11px]">
      ...
      {(selected || (selectedAtom && isTombstoned)) && (
        <div className="absolute inset-x-0 bottom-6 mx-auto w-full max-w-lg">
          <div
          ...
          >
            <div className="mb-2 flex items-start gap-2">
              {/* Close button — fixed top-left, never shifts with name length */}
              <button
              ...
              <div className="flex min-w-0 flex-wrap items-center gap-2.5">
                <span
                  className={`font-mono text-sm font-semibold break-all ${isTombstoned ? "text-slate-500" : ""}`}
                  style={
                    isTombstoned
                      ? undefined
                      : {
            ...
            {selected && (
              <>
                <div className="grid grid-cols-3 gap-3 font-mono text-[11px] text-slate-400">
                  <div>
